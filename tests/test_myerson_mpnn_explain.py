...
def regression_setup(device):
    ...
    solution = {0: 0.20805272853800247,
                1: -0.2598040096816563,
                2: -0.618848564511254,
                3: -0.2598039262351536,
                4: 0.20805272853800247,
                5: 1.5619634935898452,
                6: 2.326792989139044}
...
def classification_setup(device):
    ...
    graph = dataset[0].mg
    solution = {0: torch.tensor([[0.1456, 0.3633, 0.4215]]),
                1: torch.tensor([[ 0.0160, -0.0136,  0.4900]]),
                2: torch.tensor([[-0.0356, -0.2984,  0.0254]]),
                3: torch.tensor([[-0.0075,  0.1749,  0.0371]]),
                4: torch.tensor([[-0.0142,  0.1795,  0.0070]]),
                5: torch.tensor([[-0.0165,  0.1813,  0.0044]]),
                6: torch.tensor([[0.0187, 0.1490, 0.0131]]),
                7: torch.tensor([[ 0.2241, -0.5621, -0.2310]]),
                8: torch.tensor([[ 0.6695, -0.1739,  0.2326]])}
...
class TestMyersonSamplingClassExplainer:
    ...
    def test_with_restrict(self, classification_setup):
        ...
        my_values = sampler.sample_all_myerson_values()
        for my, sol in zip(my_values.values(), solution.values()):
            assert my == pytest.approx(sol.squeeze(), rel=1e-1, abs=1e-1), f"{my_values=}, {solution=}"